// cloudflare/build-info.ts
var BUILD_COMMIT = "local";
...
var results = (s) => [...s.players].sort(
  (a, b) => b.kills - a.kills || a.deaths - b.deaths || b.damage - a.damage
);
...
var nickname = (value) => typeof value === "string" && value.trim().length > 0 && [
  ...new Intl.Segmenter("ko", { granularity: "grapheme" }).segment(
    value.trim()
  )
].length <= 12;
var lobby = (env) => env.LOBBY.get(env.LOBBY.idFromName("public"), { locationHint: "apac" });
var room = (env, id) => env.ROOMS.get(env.ROOMS.idFromName(id), { locationHint: "apac" });
var worker_default = {
  async fetch(request, env) {
    try {
      const url = new URL(request.url);
      const requestOrigin = request.headers.get("origin");
      if (requestOrigin && requestOrigin !== env.SITE_ORIGIN)
        return json({ error: "\uD5C8\uC6A9\uB418\uC9C0 \uC54A\uC740 \uC811\uC18D" }, 403);
      ...
      if (!env.REALTIME_SECRET)
        return json({ error: "\uC2E4\uC2DC\uAC04 \uC11C\uBC84 \uC124\uC815 \uB300\uAE30" }, 503);
      if (url.pathname === "/socket") {
        if (request.headers.get("origin") !== env.SITE_ORIGIN)
          return json({ error: "\uD5C8\uC6A9\uB418\uC9C0 \uC54A\uC740 \uC811\uC18D" }, 403);
        if (request.headers.get("upgrade")?.toLowerCase() !== "websocket")
          return json({ error: "WebSocket \uD544\uC694" }, 426);
        const ticket = await verifyTicket(
          url.searchParams.get("ticket") ?? "",
          env.REALTIME_SECRET
        );
        if (!ticket) return json({ error: "\uC811\uC18D\uAD8C\uC774 \uB9CC\uB8CC\uB418\uC5C8\uC2B5\uB2C8\uB2E4" }, 401);
        return room(env, ticket.room).fetch(
          new Request("https://room/socket", {
            headers: { upgrade: "websocket", "x-owner": ticket.owner }
          })
        );
      }
      if (request.method !== "POST" || request.headers.get("authorization") !== `Bearer ${env.REALTIME_SECRET}`)
        return json({ error: "\uC778\uC99D \uD544\uC694" }, 401);
      const raw = await request.text();
      if (raw.length > 8192) return json({ error: "\uC694\uCCAD\uC774 \uB108\uBB34 \uD07D\uB2C8\uB2E4" }, 413);
      const body = JSON.parse(raw), path = url.pathname.slice(5), owner = body.owner;
      if (typeof owner !== "string" || owner.length > 80)
        return json({ error: "\uC0AC\uC6A9\uC790 \uD655\uC778 \uC2E4\uD328" }, 400);
      if (path === "list")
        return lobby(env).fetch(new Request("https://lobby/list"));
      if (path === "create") {
        if (!nickname(body.nickname))
          return json({ error: "\uB2C9\uB124\uC784\uC744 \uD655\uC778\uD574 \uC8FC\uC138\uC694" }, 400);
        const reserved = await lobby(env).fetch(
          new Request("https://lobby/create", {
            method: "POST",
            body: JSON.stringify({ owner, nickname: body.nickname })
          })
        );
        if (!reserved.ok) return reserved;
        const entry = await reserved.json();
        body.id = entry.id;
      }
      if (!/^[A-F0-9]{8}$/.test(body.id))
        return json({ error: "\uBC29 \uCF54\uB4DC\uB97C \uD655\uC778\uD574 \uC8FC\uC138\uC694" }, 400);
      const result = await room(env, body.id).fetch(
      ...
      if (path === "create" || path === "join") {
        const ticket = await signTicket(
          { room: body.id, owner, expires: Date.now() + 36e5 },
          env.REALTIME_SECRET
        );
        data.realtime = {
          url: `${url.origin.replace(/^http/, "ws")}/socket?ticket=${ticket}`
        };
      }
      return json(data);
    } catch {
      return json({ error: "\uC2E4\uC2DC\uAC04 \uC11C\uBC84 \uC5F0\uACB0 \uC624\uB958" }, 500);
    }
  }
};
var ArenaLobby = class extends DurableObject {
  entries = [];
  constructor(ctx, env) {
    super(ctx, env);
    ctx.blockConcurrencyWhile(async () => {
      this.entries = await ctx.storage.get("rooms") ?? [];
    });
  }
  async fetch(request) {
    const now = Date.now(), path = new URL(request.url).pathname;
    this.entries = this.entries.filter((e) => now - e.updated < 6e4);
    if (path === "/list")
      return json({
        rooms: this.entries.map((e) => ({
          id: e.id,
          name: e.name,
          count: e.count,
          full: e.full,
    ...
    if (path === "/create") {
      if (this.entries.length >= 16)
        return json(
          { error: "\uBB34\uB8CC \uC11C\uBC84\uC758 \uBC29\uC774 \uAC00\uB4DD \uCC3C\uC2B5\uB2C8\uB2E4. \uC7A0\uC2DC \uD6C4 \uCC38\uAC00\uD574 \uC8FC\uC138\uC694." },
          429
        );
      if (this.entries.filter(
        (e) => e.owner === body.owner && now - e.created < 6e4
      ).length >= 3)
        return json({ error: "\uBC29 \uC0DD\uC131\uC740 \uC7A0\uC2DC \uD6C4 \uB2E4\uC2DC \uC2DC\uB3C4\uD574 \uC8FC\uC138\uC694" }, 429);
      const entry = {
        id: crypto.randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase(),
        owner: body.owner,
        name: `${body.nickname}\uC758 \uC11C\uBC14\uC774\uBC8C`,
        count: 1,
        full: false,
        phase: "waiting",
        round: 1,
      ...
      await this.ctx.storage.put("rooms", this.entries);
      return json(entry);
    }
    if (path === "/update") {
      const previous = this.entries.find((e) => e.id === body.id);
      this.entries = this.entries.filter((e) => e.id !== body.id);
      if (body.count > 0)
        this.entries.push({
          ...body,
    ...
    return json({ error: "\uACBD\uB85C \uC5C6\uC74C" }, 404);
  }
};
var ArenaRoom = class extends DurableObject {
  state = null;
  roomId = "";
  timer = null;
  lastSave = 0;
  ...
  tag(ws) {
    let tag = this.tags.get(ws);
    if (!tag) {
      tag = ws.deserializeAttachment();
      this.tags.set(ws, tag);
    }
    return tag;
  }
  constructor(ctx, env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      ...
      this.outbox = await ctx.storage.get("outbox") ?? [];
      if (ctx.getWebSockets().length) this.run();
    });
  }
  snapshot(owner, cursor = 0) {
    const s = this.state;
    const safe = {
      ...s,
      seed: 0,
      players: s.players.map((p) => ({
        ...
        owner: "",
        hitBy: {},
        input: p.owner === owner ? p.input : idleInput()
      }))
    };
    return {
      id: this.roomId,
      you: s.players.find((p) => p.owner === owner)?.id ?? "",
      serverTime: Date.now(),
      ...compactState(safe, cursor)
    };
  }
  run() {
    if (this.state?.phase === "waiting") return;
    if (!this.timer) this.timer = setInterval(() => this.tick(), 1e3 / 30);
  }
  tick() {
    ...
    if (s.phase === "results" && s.recordedRound !== s.round) {
      this.outbox.push({
        id: this.roomId,
        round: s.round,
        ended: s.end,
        players: results(s).map((p) => ({
          id: p.id,
          owner: p.owner,
          name: p.name,
          kills: p.kills,
      ...
      s.recordedRound = s.round;
      this.ctx.waitUntil(this.ctx.storage.put("outbox", this.outbox));
    }
    if (s.phase === "results" && now >= s.next) {
      s.players = s.players.filter((p) => p.online);
      s.round++;
      if (s.players.length >= 2) startArena(s, now);
      else {
        s.phase = "waiting";
        s.bullets = [];
        s.drops = [];
    ...
    if (s.tick % 2 === 0 || s.phase !== "live") this.broadcast();
    if (now - this.lastSave > 5e3) {
      this.lastSave = now;
      this.ctx.waitUntil(this.persist());
    }
    if (!this.ctx.getWebSockets().length || s.phase === "waiting") {
  ...
  async persist() {
    ...
    try {
      do {
        ...
        const s = this.state, players = s.players.filter((p) => p.online);
        await lobby(this.env).fetch(
          new Request("https://lobby/update", {
            method: "POST",
            body: JSON.stringify({
              id: this.roomId,
              owner: s.players[0]?.owner ?? "",
              name: `${s.players[0]?.name ?? "\uB300\uAE30"}\uC758 \uC11C\uBC14\uC774\uBC8C`,
              count: players.length,
              full: s.players.length >= 8,
              phase: s.phase,
              round: s.round,
              updated: Date.now(),
              created: Date.now()
  ...
  async alarm() {
    await this.report();
    if (this.outbox.length) await this.ctx.storage.setAlarm(Date.now() + 3e4);
  }
  async report() {
    if (this.reporting || !this.outbox.length) return;
    this.reporting = true;
    try {
      const pending = this.outbox[0];
      const response = await fetch(`${this.env.SITE_ORIGIN}/api/pvp/results`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${this.env.REALTIME_SECRET}`,
      ...
      if (response.ok) {
        this.outbox.shift();
        await this.ctx.storage.put("outbox", this.outbox);
      }
    } catch {
    } finally {
  ...
  broadcast() {
    for (const ws of this.ctx.getWebSockets()) {
      const tag = this.tag(ws);
      try {
        ws.send(
          JSON.stringify({
            type: "snapshot",
            ...this.snapshot(tag.owner, tag.cursor)
          })
        );
        tag.cursor = this.state.events.reduce(
          (n, e) => Math.max(n, e.id),
          tag.cursor
        );
      } catch {
        try {
          ws.close(1011, "\uC5F0\uACB0 \uC7AC\uC2DC\uB3C4");
        } catch {
        }
      }
    }
  ...
  async fetch(request) {
    const path = new URL(request.url).pathname, owner = request.headers.get("x-owner") ?? "";
    if (path === "/socket") {
      const player = this.state?.players.find((p) => p.owner === owner);
      if (!player || !player.online && player.seen === 0)
        return json({ error: "\uBA3C\uC800 \uBC29\uC5D0 \uCC38\uAC00\uD574 \uC8FC\uC138\uC694" }, 403);
      for (const ws of this.ctx.getWebSockets())
        if (ws.deserializeAttachment().owner === owner)
          ws.close(1e3, "\uC0C8 \uC5F0\uACB0");
    ...
    const body = await request.json();
    if (path === "/create" && !this.state) {
      if (!nickname(body.nickname)) return json({ error: "\uB2C9\uB124\uC784 \uD655\uC778" }, 400);
      this.roomId = String(body.id);
      this.state = createArena(Date.now(), parseInt(this.roomId, 16));
      addFighter(
        this.state,
        owner,
      ...
      if (!this.state || !nickname(body.nickname))
        return json({ error: "\uBC29 \uB610\uB294 \uB2C9\uB124\uC784 \uD655\uC778" }, 404);
      if (!this.ctx.getWebSockets().length && Date.now() - this.state.updated > 6e4)
        return json({ error: "\uC885\uB8CC\uB41C \uBC29\uC785\uB2C8\uB2E4" }, 404);
      try {
        addFighter(
          this.state,
          owner,
        ...
        return json({ error: "\uBC29\uC774 \uAC00\uB4DD \uCC3C\uC2B5\uB2C8\uB2E4" }, 409);
      }
    } else {
      const error = this.command(owner, path.slice(1), body);
      if (error) return json({ error }, 400);
    }
    this.run();
    this.broadcast();
    this.ctx.waitUntil(this.persist());
    return json(this.snapshot(owner));
  }
  command(owner, action, body) {
    const s = this.state, p = s?.players.find((p2) => p2.owner === owner);
    if (!s || !p) return "\uBC29\uC5D0 \uCC38\uAC00\uD574 \uC8FC\uC138\uC694";
    p.seen = Date.now();
    if (action === "ready") p.ready = body.ready === true;
    else if (action === "start") {
      const active = s.players.filter((p2) => p2.online);
      if (active[0]?.owner !== owner || s.phase !== "waiting" || active.length < 2 || !active.every((p2) => p2.ready))
        return "\uBC29\uC7A5\uACFC \uC900\uBE44 \uC0C1\uD0DC\uB97C \uD655\uC778\uD574 \uC8FC\uC138\uC694";
      s.players = active;
      startArena(s, Date.now());
    } else if (action === "leave") {
      leaveArena(s, p);
      for (const ws of this.ctx.getWebSockets())
        if (ws.deserializeAttachment().owner === owner)
          ws.close(1e3, "\uD1F4\uC7A5");
    } else if (action !== "sync") return "\uC9C0\uC6D0\uD558\uC9C0 \uC54A\uB294 \uC694\uCCAD\uC785\uB2C8\uB2E4";
    return "";
  }
  webSocketMessage(ws, message) {
    ...
    try {
      ...
      const p = this.state?.players.find((p2) => p2.owner === tag.owner);
      if (!p) return;
      p.seen = now;
      p.online = true;
      const data = JSON.parse(message);
      if (data.type === "ping")
        ws.send(JSON.stringify({ type: "pong", sent: data.sent }));
      else if (data.type === "input") {
        const input = sanitizeInput(data.input);
        if (input.seq > p.input.seq) {
          input.dash ||= p.input.dash;
          input.reload ||= p.input.reload;
          p.input = input;
          p.inputAt = now;
        }
      } else if (data.type === "command") {
        const error = this.command(tag.owner, data.action, data.body ?? {});
        if (data.action !== "leave")
          ws.send(
            JSON.stringify({
              ...
              error: error || void 0,
              data: error ? void 0 : this.snapshot(tag.owner)
            })
          );
        this.broadcast();
        this.ctx.waitUntil(this.persist());
      }
      if (this.state?.phase === "waiting" && now - this.lastSave > 15e3) {
        this.lastSave = now;
        advanceArena(this.state, now);
        this.broadcast();
        this.ctx.waitUntil(this.persist());
      }
      this.run();
    } catch {
      ws.close(1008, "\uC62C\uBC14\uB974\uC9C0 \uC54A\uC740 \uC694\uCCAD");
    }
  }
  webSocketClose(ws, code, reason) {
    ...
    const stillConnected = this.ctx.getWebSockets().some(
      (other) => other !== ws && other.deserializeAttachment().owner === tag.owner
    );
    if (!stillConnected) {
      const p = this.state?.players.find((p2) => p2.owner === tag.owner);
      if (p) p.input = idleInput();
    }
    this.ctx.waitUntil(this.persist());
  }
  webSocketError(ws) {
